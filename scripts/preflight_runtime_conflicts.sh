#!/usr/bin/env bash
# preflight_runtime_conflicts.sh — Detect runtime conflicts before launching mapping.
...
# ---------- 6. SuperOdom not running (mapping conflict) ----------
SO_PROCS=$(pgrep -a -f '(feature_extraction_node|laser_mapping_node|imu_preintegration_node)' 2>/dev/null || true)
if [[ -n "$SO_PROCS" ]]; then
    warn "SuperOdom processes running — will conflict with liorf if both use /velodyne_points"
    echo "$SO_PROCS" | sed 's/^/       /'
else
    pass "No SuperOdom processes running"
fi
...
echo ""
if [[ $FAIL -eq 0 ]]; then
    echo -e "${GRN}Preflight clean — safe to launch mapping.${RST}"
    exit 0
else
